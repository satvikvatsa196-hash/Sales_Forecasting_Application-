...
flowchart LR
    ...
    UI --> Data[Data Layer<br/>data_generator.py]
    UI --> Models[Forecasting Models<br/>forecasting_models.py]
    UI --> Viz[Visualization Layer<br/>visualizations.py]
    Data --> Models
    Models --> Metrics[Evaluation Metrics]
    Metrics --> Viz
    Viz --> Output[Forecast Charts<br/>Metrics Table<br/>CSV Export]
```
...
sequenceDiagram
    ...
    participant M as Forecasting Models
    participant V as Visualization Layer

    U->>S: Select data source and forecast settings
    S->>D: Load or generate sales data
    D-->>S: Cleaned and prepared dataframe
    S->>M: Run selected forecasting models
    M-->>S: Predictions and evaluation metrics
    S->>V: Build charts and comparison views
    V-->>S: Interactive plots and tables
    S-->>U: Display forecasts and insights
```
